...
def preprocess(gene_trainsets, protein_trainsets, gene_test = None,
               train_batchkeys = None, test_batchkey = None, type_key = None,
               gene_list = [], select_hvg = True, cell_normalize = True, log_normalize = True,
    ...
    if train_batchkeys is not None:
        ...
        for i in range(len(gene_trainsets)):
            ...
            gene_trainsets[i].obs['batch'] = gene_trainsets[i].obs['batch'].astype("category")
            protein_trainsets[i].obs['batch'] = ['DS-' + str(i + 1) + ' ' + x for x in protein_trainsets[i].obs[key]]
            protein_trainsets[i].obs['batch'] = protein_trainsets[i].obs['batch'].astype("category")
    else:
        for i in range(len(gene_trainsets)):
